/*
 *Q4.6 Design an algorithm and write code to find the first common ancestor of two nodes in a binary tree.
 *Avoid storing addtional nodes ina data strcucture.
...
void create_minial_tree(Node* &head, Node *parent, int a[], int start, int end)
{
	if (start <= end)
	{
		int mid=(start + end)>>1;
...
Node* search(Node* head, int x)
{
	...
	else if (x == head->key) return NULL;
...
int main()
{
	...
	int a[] = { 0,1,2,3,4,5,6 };
	Node *head = NULL;
	create_minial_tree(head,NULL,a,0,6);
